...
### --------------------------------------------------------------------------------------------------------- ###
parser = argparse.ArgumentParser(description="Generate content evaluations made by VLM personas/baselines")
parser.add_argument(
    "--inputfile",
    required=True,
...
parser.add_argument(
    "--outputfile",
    required=True,
    help="File name of .csv file containing content evaluations made by VLM personas/baselines"
)
parser.add_argument(
    "--systemprompts",
    required=True,
...
parser.add_argument(
    "--imgcol",
    required=True,
    help="Name of the column in the CSV containing the image to evaluate"
)
parser.add_argument(
    "--model",
    required=True,
    help="Model name as on Fireworks AI"
)
parser.add_argument(
    "--id",
    required=True,
    help="Deployment id as on Fireworks AI"
)
parser.add_argument(
    "--baseline",
    action="store_true",
...
def inference(
    ...
    This leverages code from https://fireworks.ai/docs/guides/querying-vision-language-models
...
def main() -> None:
    ...
    for persona, prompt_template in product(chain.from_iterable(personas.values()), templates):
        ...
        data[[f"{p}_response_json", f"{p}_response_text"]] = data.apply(
            lambda row: inference(
                image_path=row[args.imgcol],
    ...
    if args.baseline:
        data[["baseline_response_json", "baseline_response_text"]] = data.apply(
            lambda row: inference(
                image_path=row[args.imgcol],
                user_prompt=user_prompts["user_prompt"],
            ),
