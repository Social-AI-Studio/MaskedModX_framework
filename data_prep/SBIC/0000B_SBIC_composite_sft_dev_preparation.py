...
### --------------------------------------------------------------------------------------------------------- ###
SBIC_dev = pd.read_csv(
    "SBIC.v2/SBIC.v2.dev.csv",
    encoding="utf-8"
)
SBIC_dev = SBIC_dev.replace(["na", ""], np.nan).dropna(subset=[
    "annotatorGender", "annotatorRace", "annotatorPolitics", "annotatorAge"]).reset_index(drop=True)
SBIC_dev = SBIC_dev.drop_duplicates(subset=["WorkerId", "HITId", "annotatorGender", "annotatorRace", "annotatorPolitics", "annotatorAge"], keep="first").reset_index(drop=True)
SBIC_dev["offensiveYN"] = SBIC_dev["offensiveYN"].replace({1.0: "Yes", 0.5: "Maybe", 0.0: "No"}).fillna("Incomprehensible")
lines = []
SBIC_dev["annotatorGender"] = SBIC_dev["annotatorGender"].replace({"nonBinary": "non-binary individual",
                                                                   "transman": "transgender man"})
SBIC_dev["annotatorRace"] = SBIC_dev["annotatorRace"].replace({"asian": "Asian/Asian American",
...
SBIC_dev["annotatorPolitics"] = SBIC_dev["annotatorPolitics"].replace({"cons": "a conservative political leaning",
...
for _, row in SBIC_dev.iterrows():
...
with open("SBIC_dev.jsonl", "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
